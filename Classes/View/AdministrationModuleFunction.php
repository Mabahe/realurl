...
class AdministrationModuleFunction extends \TYPO3\CMS\Backend\Module\AbstractFunctionModule {
	...
	function logView()	{
		...
		if (is_array($list))	{
			...
			foreach($list as $rec)	{
				...
				$newRedirectLinkOnClick = \TYPO3\CMS\Backend\Utility\BackendUtility::editOnClick('&edit[tx_realurl_redirects]['.$this->pObj->id.']=new&defVals[tx_realurl_redirects][url]='.rawurlencode($rec['url']),$this->pObj->doc->backPath);

				$newRedirectLink = '<a href="#" onclick="'.htmlspecialchars($newRedirectLinkOnClick).'">' .
					'<img' . \TYPO3\CMS\Backend\Utility\IconUtility::skinImg($this->pObj->doc->backPath,'gfx/napshot.gif','width="16" height="16"').' title="Set as redirect" alt="" />
				</a>';
	...
	function redirectView()	{
		...
		$output = $this->newRedirectView();

		return $output;
	}

	function newRedirectView() {
		// Init variables.
		$output='';
		...
		$list = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
			...
			'tx_realurl_redirects.pid = ' . (int)\TYPO3\CMS\Core\Utility\GeneralUtility::_GP('id') . ' AND tx_realurl_redirects.deleted = 0 ',
			'',
		...
		if (is_array($list)) {
			...
			foreach($list as $rec)	{
				$editIconOnClick = \TYPO3\CMS\Backend\Utility\BackendUtility::editOnClick('&edit[tx_realurl_redirects]['.$rec['uid'].']=edit',$this->pObj->doc->backPath);
				...
				if(empty($rec['hidden'])) {
					$hideUnhideButton = '<a href="#" onclick="jumpToUrl(\''.htmlspecialchars($this->pObj->doc->issueCommand('&data[tx_realurl_redirects]['.$rec['uid'].'][hidden]=1')).'\');return false;">
								<img'.\TYPO3\CMS\Backend\Utility\IconUtility::skinImg($this->pObj->doc->backPath,'gfx/button_hide.gif','width="16" height="16"').' title="Hide entry" alt="" />
							</a>';
				} else {
					$hideUnhideButton = '<a href="#" onclick="jumpToUrl(\''.htmlspecialchars($this->pObj->doc->issueCommand('&data[tx_realurl_redirects]['.$rec['uid'].'][hidden]=0')).'\');return false;">
								<img'.\TYPO3\CMS\Backend\Utility\IconUtility::skinImg($this->pObj->doc->backPath,'gfx/button_unhide.gif','width="16" height="16"').' title="Unhide entry" alt="" />
							</a>';
				}
				...
				$tCells[] = '<td>'.
					...
					'<img'.\TYPO3\CMS\Backend\Utility\IconUtility::skinImg($this->pObj->doc->backPath,'gfx/edit2.gif','width="16" height="16"').' title="Edit entry" alt="" />
							</a>
							'.$hideUnhideButton.
					...
					'<a onclick="if(confirm(String.fromCharCode(68,105,101,115,101,110,32,68,97,116,101,110,115,97,116,122,32,116,97,116,115,228,99,104,108,105,99,104,32,108,246,115,99,104,101,110,63))){
								jumpToUrl(\''.htmlspecialchars($this->pObj->doc->issueCommand('&cmd[tx_realurl_redirects]['.$rec['uid'].'][delete]=1')).'\');};return false;" href="#" title="' . $GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_core.php:cm.delete', TRUE) . '"><img'.\TYPO3\CMS\Backend\Utility\IconUtility::skinImg($this->pObj->doc->backPath,'gfx/garbage.gif','width="16" height="16"').' title="Delete entry" alt="" /></a>'.
					'</td>';
				$tCells[]='<td>'.$rec['counter'].'</td>';
				...
				if(!empty($rec['sys_domain_domainName'])) {
					...
					if(!empty($rec['url'])) {
						$tCells[] = sprintf( '<td><a href="%s" target="_blank">%s</a></td>',htmlspecialchars(\TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL').$rec['url']), htmlspecialchars($rec['url']) );
					} else {
				...
				if(!empty($rec['destination'])) {
					$tCells[] = sprintf( '<td><a href="%s" target="_blank" title="%s">%s</a></td>', htmlspecialchars(\TYPO3\CMS\Core\Utility\GeneralUtility::locationHeaderUrl($rec['destination'])), htmlspecialchars($rec['destination']), ( strlen( htmlspecialchars($rec['destination']) ) > 30 ) ? substr(htmlspecialchars($rec['destination']),0,30) . '...' : htmlspecialchars($rec['destination']) );
				} else {
				...
				if(!empty($rec['last_time'])) {
					$tCells[]='<td>'.\TYPO3\CMS\Backend\Utility\BackendUtility::dateTimeAge($rec['last_time']).'</td>';
				} else {
					$tCells[]='<td>'.\TYPO3\CMS\Backend\Utility\BackendUtility::dateTimeAge($rec['tstamp']).'</td>';
				}
			...
			$newIconOnClick = \TYPO3\CMS\Backend\Utility\BackendUtility::editOnClick('&edit[tx_realurl_redirects]['.$this->pObj->id.']=new',$this->pObj->doc->backPath);

			// Compile final table and return:
			$output = '<br/>
				<a href="#" onclick="'.htmlspecialchars($newIconOnClick).'">'.
				'<img'.\TYPO3\CMS\Backend\Utility\IconUtility::skinImg($this->pObj->doc->backPath,'gfx/new_el.gif','width="16" height="16"').' title="New entry" alt="" />
				</a><br/>
				<table border="0" cellspacing="1" cellpadding="0" id="tx-realurl-pathcacheTable" class="lrPadding c-list">'.$output.'
				</table>';
